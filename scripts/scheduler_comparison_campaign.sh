...
SESSION="scheduler-comparison-dev-n300-s1001-v1"
CAMPAIGN_ID="scheduler_comparison_dev_n300_seed1001_v1"
CAMPAIGN_ROOT="${REPO_ROOT}/results/raw/qwen3_14b_dgx_spark/${CAMPAIGN_ID}"

usage() {
  cat <<'EOF'
Usage: scripts/scheduler_comparison_campaign.sh COMMAND [--resource-approved]

Commands:
  preview                    Print the smoke gate and fixed six-run matrix.
  smoke --resource-approved  Run/repair the required Stock+DPP smoke in detached tmux.
  start --resource-approved  Launch six runs only after the smoke checkpoint passed.
  status                     Print tmux and append-only checkpoint status.
  resume --resource-approved Resume failed/pending work in detached tmux.
  validate                   Fail-closed validation and comparison report preview.

The campaign is a single-seed, non-formal engineering comparison. Run `smoke`,
review `status`, then run `start`. The main matrix cannot start without a passed
Stock+DPP smoke checkpoint. SSH disconnection does not stop either tmux worker.
EOF
}
...
resource_preflight() {
  ...
  printf 'job=%s\n' "${SESSION}"
  printf 'scope=development non-formal Stock-vs-DPP comparison\n'
  printf 'matrix=2 policies x 3 QPS x seed 1001; 300 requests per main run\n'
  printf 'smoke_gate=1 Stock + 1 DPP request before all main runs\n'
  printf 'expected_duration=4-8 hours; hard internal campaign bound=12 hours\n'
...
launch_worker() {
  ...
  printf -v shell_command 'exec %q -m benchmarks.run_scheduler_comparison_campaign %q' \
  ...
  printf 'The worker will stop before the six main runs if either smoke fails.\n'
}
...
case "${command}" in
  preview)
    ...
    "${PYTHON}" -m benchmarks.run_scheduler_comparison_campaign preview
    ;;
  smoke)
    require_approval_flag "$@"
    resource_preflight
  ...
  status)
    ...
    "${PYTHON}" -m benchmarks.run_scheduler_comparison_campaign status
    ;;
  resume)
    require_approval_flag "$@"
    if [[ ! -f "${CAMPAIGN_ROOT}/campaign_checkpoint.json" ]]; then
  ...
  validate)
    if (( $# != 0 )); then usage >&2; exit 2; fi
    "${PYTHON}" -m benchmarks.run_scheduler_comparison_campaign validate
    ;;
  -h|--help|help)
    usage
    ;;
